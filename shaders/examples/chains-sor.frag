...
//
// hacked to use successive-over-relaxation sphere tracing - EAK
void Raymarch( const in C_Ray ray, out C_HitInfo result, const int maxIter, bool forceHit, float pixelRadius )
{        
    result.fDistance   = GetRayFirstStep( ray );
    result.vObjectId.x = 0.0;

#define INFINITY 1000.0
#define forceHit false
#define t_max INFINITY
    float omega = 1.6;
    float t = result.fDistance;
    result.fError = INFINITY;
    float previousRadius = 0;
    float stepLength = 0;
    float functionSign = GetDistanceScene(ray.vOrigin).x < 0 ? -1 : +1;
    for (int i = 0; i < kRaymarchMaxIter; ++i) {
      vec3 here = ray.vOrigin + ray.vDir*t;
      vec4 vSceneDist = GetDistanceScene(here);
      float signedRadius = vSceneDist.x * functionSign;
    ...
    if ((t > t_max || result.fError > pixelRadius) && !forceHit) {
      ...
      result.vPos = ray.vOrigin + ray.vDir * INFINITY;
      result.vObjectId.x = 0.0;
    }
}
