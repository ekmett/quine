...
numAttachedShaders p = fromIntegral `liftM` getProgram p GL_ATTACHED_SHADERS

attachedShaders :: MonadIO m => Program -> m [Shader]
attachedShaders p = do
  numShaders <- fromIntegral `liftM` getProgram p GL_ATTACHED_SHADERS
...
getProgram :: MonadIO m => Program -> GLenum -> m GLint
getProgram s p = liftIO $ alloca $ \q -> glGetProgramiv (object s) p q >> peek q
...
programInfoLog p = liftIO $ do
  l <- fromIntegral <$> getProgram p GL_INFO_LOG_LENGTH
  if l <= 1
...
programIsDeleted p = (GL_FALSE /=) `liftM` getProgram p GL_DELETE_STATUS
...
linkStatus p = (GL_FALSE /=) `liftM` getProgram p GL_LINK_STATUS
...
validateStatus p = (GL_FALSE /=) `liftM` getProgram p GL_VALIDATE_STATUS
...
activeAtomicCounterBuffers p = fromIntegral `liftM` getProgram p GL_ACTIVE_ATOMIC_COUNTER_BUFFERS
...
numActiveAttributes p = fromIntegral `liftM` getProgram p GL_ACTIVE_ATTRIBUTES
...
activeAttributeMaxLength p = fromIntegral `liftM` getProgram p GL_ACTIVE_ATTRIBUTE_MAX_LENGTH

-- * Uniforms

-- | @'numActiveUniforms' returns the number of active uniform variables for @program@.
numActiveUniforms :: MonadIO m => Program -> m Int
numActiveUniforms p = fromIntegral `liftM` getProgram p GL_ACTIVE_UNIFORMS
...
activeUniformMaxLength p = fromIntegral `liftM` getProgram p GL_ACTIVE_ATTRIBUTE_MAX_LENGTH
...
programBinaryLength p = fromIntegral `liftM` getProgram p GL_PROGRAM_BINARY_LENGTH
...
transformFeedbackBufferMode p = fromIntegral `liftM` getProgram p GL_TRANSFORM_FEEDBACK_BUFFER_MODE
...
numTransformFeedbackVaryings p = fromIntegral `liftM` getProgram p GL_TRANSFORM_FEEDBACK_VARYINGS
...
transformFeedbackVaryingsMaxLength p = fromIntegral `liftM` getProgram p GL_TRANSFORM_FEEDBACK_VARYINGS
...
geometryVerticesOut p = fromIntegral `liftM` getProgram p GL_GEOMETRY_VERTICES_OUT
...
geometryInputType p = fromIntegral `liftM` getProgram p GL_GEOMETRY_INPUT_TYPE
...
geometryOutputType p = fromIntegral `liftM` getProgram p GL_GEOMETRY_OUTPUT_TYPE
