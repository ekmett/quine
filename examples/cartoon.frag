...
// @var headSize 0.2
// @var eyePos -1.0000411 0.29724762 -5.1204014
// @todo NPOT texture support: var tex0 tex14.png
...
vec3 path(float ti) {
	ti*=1.5;
	// Eliminate camera motion for use within Oculus Rift
	ti = 0.0;
...
vec4 nyan(vec2 p)
{
	...
	vec4 color = vec4(0.0); //texture2D(iChannel1,vec2(uv.x/3.0+210.0/256.0-nt+0.05,.5-uv.y-ny));
...
void main(void)
{
	...
	vec3 color = getSceneColor( origin, dir );
